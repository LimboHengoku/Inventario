...
public class UsuarioServiceImpl implements UsuarioService {
	...
	public LoginResponse login(String idTrans, LoginRequest request) {
		...
		try {
			...
			Logueo login = usuarioDAO.login(idTrans, 
	...
	public RegistrarUsuarioResponse registrarUsuario(String idTrans, RegistrarUsuarioRequest request) {
		...
		try {
			...
			Map<String,String> resultUsuario = usuarioDAO.registrarUsuario(idTrans, 
					u);
			
			if(resultUsuario.get(Constantes.CODIGO_RESPUESTA).toString()
					.equals(Constantes.CODIGO_OK)) {
				
				Map<String,String> resultUbicacion = ubicacionDAO.
						registrarUbicacion(idTrans, ub);
				...
				if(resultUbicacion.get(Constantes.CODIGO_RESPUESTA).toString()
						.equals(Constantes.CODIGO_OK)) {
					
					Map<String,String> params = new HashMap<>();
					params.put(Constantes.ID_CONDICION, String.valueOf(request.getUsuario()
							.getUsuarioUbicacion().getCondicionBean().getIdCondicion()));
					params.put(Constantes.ID_DOMINIO, String.valueOf(request.getUsuario()
							.getUsuarioUbicacion().getDominioBean().getIdDominio()));
					params.put(Constantes.ID_UBICACION, String.valueOf(resultUbicacion
							.get(Constantes.CODIGO_GENERADO).toString()));
					params.put(Constantes.ID_USUARIO, String.valueOf(resultUsuario
							.get(Constantes.CODIGO_GENERADO).toString()));
					...
					Map<String,String> resultUsuUbi = usuarioDAO.registrarUbicacionUsuario(idTrans, 
							params);
					
					if(resultUsuUbi.get(Constantes.CODIGO_RESPUESTA).toString()
							.equals(Constantes.CODIGO_OK)) {
